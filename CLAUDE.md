...
| [docs/teaching-style-rules.md](docs/teaching-style-rules.md) | **Always follow these rules.** When explaining any concept — start with what the student already knows, show the problem first, introduce the solution in plain English before giving the technical term, then explain the real mechanics so they finish at advanced level. Never use jargon without defining it first; never dumb it down — build it up. |
